...
export default class Hen implements IHen {
	...
	public move = (blocksAroundPoint: any): PlayerResultEnum[] => {
		...
		if (this.direction === DirectionEnum.EATING_RIGHT || this.direction === DirectionEnum.EATING_LEFT)
			result.push(this.eatFood());
	...
	private eatFood = (): PlayerResultEnum => {
		this.updateImage();
		if (this.direction === DirectionEnum.EATING_RIGHT && this.imageIteration === 1)
		...
		if (this.direction === DirectionEnum.EATING_RIGHT && this.imageIteration === this.images[this.direction].length - 1)
			this.direction = DirectionEnum.RIGHT;
		if (this.direction === DirectionEnum.EATING_LEFT && this.imageIteration === this.images[this.direction].length - 1)
	...
	private checkIfDirectionValid = (x: number, y: number, blocksAroundHen: IBlockPosition, isOnBlock: boolean): any => {
		const result: PlayerResultEnum[] = [];

		switch (this.direction) {
			case DirectionEnum.UP:
				result.push(this.checkUp(y, blocksAroundHen, isOnBlock));
				break;
			case DirectionEnum.RIGHT:
				result.push(this.checkRight(blocksAroundHen, isOnBlock));
				break;
			case DirectionEnum.DOWN:
				result.push(this.checkDown(blocksAroundHen, isOnBlock));
				break;
			case DirectionEnum.LEFT:
				result.push(this.checkLeft(x, blocksAroundHen, isOnBlock));
				break;
		}
	...
	private checkDown = (blocksAroundHen: IBlockPosition, _isOnBlock: boolean): PlayerResultEnum => {
		if (
	...
	private checkRight = (blocksAroundHen: IBlockPosition, isOnBlock: boolean): PlayerResultEnum => {
		if (isOnBlock && blocksAroundHen[DirectionEnum.FLOOR_RIGHT] === SpriteTypeEnum.FOOD)
			return PlayerResultEnum.HEN_EATING;
	...
	private checkLeft = (x: number, blocksAroundHen: IBlockPosition, isOnBlock: boolean): PlayerResultEnum => {
		if (isOnBlock && blocksAroundHen[DirectionEnum.FLOOR_LEFT] === SpriteTypeEnum.FOOD)
			return PlayerResultEnum.HEN_EATING;
	...
	private randomAvailableDirection = (blocksAroundHen: IBlockPosition): DirectionEnum => {
		...
		if (this.direction !== DirectionEnum.DOWN && blocksAroundHen[DirectionEnum.UP] === SpriteTypeEnum.LADDER)
			availableDirections.push(DirectionEnum.UP);
		if (this.direction !== DirectionEnum.RIGHT && blocksAroundHen[DirectionEnum.DOWN_RIGHT] === SpriteTypeEnum.FLOOR)
			availableDirections.push(DirectionEnum.RIGHT);
		if (this.direction !== DirectionEnum.UP && blocksAroundHen[DirectionEnum.DOWN] === SpriteTypeEnum.LADDER)
			availableDirections.push(DirectionEnum.DOWN);
		if (this.direction !== DirectionEnum.LEFT && blocksAroundHen[DirectionEnum.DOWN_LEFT] === SpriteTypeEnum.FLOOR)
			availableDirections.push(DirectionEnum.LEFT);
