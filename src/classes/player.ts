...
export default class Player implements IPlayer {
	...
	private checkIfDirectionValid = (
		direction: DirectionEnum,
		x: number,
		y: number,
		blocksAroundPoint: any,
		isOnBlock: boolean,
	): any => {
		...
		switch (direction) {
			case DirectionEnum.UP:
				result = this.checkUp(blocksAroundPlayer, isOnBlock);
				break;
			case DirectionEnum.RIGHT:
				result = this.checkRight(x, y, blocksAroundPlayer, isOnBlock);
				break;
			case DirectionEnum.DOWN:
				result = this.checkDown(blocksAroundPlayer, isOnBlock);
				break;
			case DirectionEnum.LEFT:
				result = this.checkLeft(x, y, blocksAroundPlayer, isOnBlock);
				break;
			case DirectionEnum.FALL_DOWN:
				result = this.fall(x, y, blocksAroundPlayer);
				break;
			case DirectionEnum.JUMP:
				result = this.jump(x, y, blocksAroundPoint);
				break;
	...
	private jump = (x: number, y: number, blocksAroundPoint: any): any => {
		const result = { validMove: false, outcome: this.isJumping ? [] : [PlayerResultEnum.START_JUMP_TIMER] };
		...
		const blocksAroundPlayer = blocksAroundPoint(x, y);
		const blockBelowWhenGoingDown =
			nextMove.direction === DirectionEnum.DOWN && blocksAroundPlayer[DirectionEnum.DOWN] === SpriteTypeEnum.FLOOR;
		const ladderBelowWhenGoingDown =
			nextMove.direction === DirectionEnum.DOWN && blocksAroundPlayer[DirectionEnum.DOWN] === SpriteTypeEnum.LADDER;
		...
		const blockAbove =
			blocksAroundPlayer[DirectionEnum.UP] === undefined ||
			blocksAroundPlayer[DirectionEnum.UP] === SpriteTypeEnum.FLOOR;
	...
	private fall = (_x: number, _y: number, blocksAroundPlayer: any): any => {
		if (
			blocksAroundPlayer[DirectionEnum.DOWN] >= SpriteTypeEnum.FLOOR &&
			blocksAroundPlayer[DirectionEnum.DOWN] <= SpriteTypeEnum.LADDER
		) {
			this.isFalling = false;
	...
	private checkForEgg = (
		blocksAroundPlayer: IBlockPosition,
		direction: DirectionEnum,
		isOnBlock: boolean,
		outcome: PlayerResultEnum[],
	): PlayerResultEnum[] => {
		if (
