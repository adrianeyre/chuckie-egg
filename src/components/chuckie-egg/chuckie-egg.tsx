...
export default class ChuckieEgg extends React.Component<IChuckieEggProps, IChuckieEggState> {
	...
	public override render() {
		return (
			<div
				className="chuckie-egg-play-container"
				ref={(d) => {
					this.container = d;
				}}
					...
					<div>
						<div className="play-area">
							<div style={this.styleStatusTop()}>
								<GameStatusTop
									score={this.state.game.board.player.score}
									lives={this.state.game.board.player.lives}
									level={this.state.game.level}
									time={this.state.game.board.time}
								/>
							</div>

							{this.state.game.board.sprites.map((sprite: ISprite) => (
								<DrawSprite
									key={sprite.key}
							...
							{this.state.game.board.hens.map((sprite: IHen) => (
								<DrawSprite
									key={sprite.key}
									onMouseOver={this.onMouseOver}
									onContextMenu={this.onContextMenu}
							...
							{this.state.game.board.lifts.map((sprite: ILift) => (
								<DrawSprite
									key={sprite.key}
									onMouseOver={this.onMouseOver}
									onContextMenu={this.onContextMenu}
							...
							<DrawSprite
								...
								onContextMenu={this.onContextMenu}
								onClick={this.onClick}
								sprite={this.state.game.board.player}
								height={this.state.spriteHeight}
								width={this.state.spriteWidth}
								containerWidth={this.state.containerWidth}
							/>
	...
	private refreshGameState = async (_result: PlayerResultEnum): Promise<void> => {
		if (!this.state.game) return;
	...
	private onMouseOver = (_event: any) => {};
	private onContextMenu = (_event: any) => {};
